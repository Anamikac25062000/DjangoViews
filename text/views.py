...
class CreateSnippetView(CreateAPIView):
    ...
    serializers_class = SnippetCreateSerializer

    def post(self, request, *args, **kwargs):
        datas = request.data
        snip_serializer = self.get_serializer(data=datas)

        if snip_serializer.is_valid():
            snip_serializer.save()
            return Response({"message":"success"}, status=200)
        return Response({"message":"Failed","errors":snip_serializer.errors}, status=400)
